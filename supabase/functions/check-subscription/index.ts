...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      logStep("No Stripe customer found, updating is_premium=false");
      await supabaseClient.from("user_profiles").update({ is_premium: false }).eq("id", user.id);
    ...
    await supabaseClient.from("user_profiles").update({ is_premium: hasActiveSub }).eq("id", user.id);
    logStep("Database synced", { is_premium: hasActiveSub });
